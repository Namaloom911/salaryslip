...
function _getTags(postParsed) {
  ...
  function processFiltered(part, current, filtered) {
    if (filtered.length) {
      stack.push({
        ...
        path: part.dataBound !== false && !part.attrParsed && part.value ? [].concat(_toConsumableArray(current.path), [part.value]) : _toConsumableArray(current.path)
      });
    }
  }
  ...
  while (stack.length > 0) {
    ...
    for (var _i2 = 0, _current$items2 = current.items; _i2 < _current$items2.length; _i2++) {
        ...
        for (var _i4 = 0, _part$cellPostParsed2 = part.cellPostParsed; _i4 < _part$cellPostParsed2.length; _i4++) {
          ...
          if (cp.type === "placeholder") {
            if (cp.module === "pro-xml-templating/xls-module-innerloop") {
              var _localTags, _cp$value;
              (_localTags = localTags)[_cp$value = cp.value] || (_localTags[_cp$value] = {});
              processFiltered(cp, current, cp.subparsed.filter(isPlaceholder));
            } else if (!cp.module) {
              var _localTags2, _cp$value2;
              var sizeScope = getScopeSize(part, current.parents);
              localTags = getLocalTags(tags, current.path, sizeScope);
              (_localTags2 = localTags)[_cp$value2 = cp.value] || (_localTags2[_cp$value2] = {});
            }
          }
        }
        continue;
      ...
      (_localTags3 = localTags)[_part$value = part.value] || (_localTags3[_part$value] = {});
      if (part.subparsed) {
        processFiltered(part, current, part.subparsed.filter(isPlaceholder));
      }
    }
